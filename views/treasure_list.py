...
def treasure_list_view(page: ft.Page):
    ...
    def load_treasures(search=""):
        return con.execute(
            "SELECT treasure_id, treasure_name, grade_code, cur_ability, series_name, image_path FROM treasure WHERE treasure_name LIKE ? ORDER BY grade_code, treasure_name",
            [f"%{search}%"]
        ).fetchall()
        ...
        for row in rows:
            ...
            data_rows.append(
                ft.DataRow(
                    ...
                    on_select_change=lambda e, tid=treasure_id: page.go(f"/treasures/{tid}"),
                )
            )
        return data_rows
